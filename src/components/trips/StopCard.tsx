...
import { MapPin, Calendar, Plus, Trash2, ArrowUp, ArrowDown, Navigation } from 'lucide-react';
...
export const StopCard: React.FC<StopCardProps> = ({
  ...
  return (
    <div style={{ position: 'relative' }}>
      <div className="glass-card animate-fade-in" style={{ marginBottom: '1.5rem', overflow: 'hidden', background: 'var(--bg-card)' }}>
        {/* Stop Header */}
        <div style={{
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', fontSize: '0.825rem', color: 'var(--text-secondary)', marginTop: '0.2rem' }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                  <Calendar size={13} style={{ color: 'var(--primary)' }} /> {stop.startDate} → {stop.endDate}
                </span>
                <span style={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                  <MapPin size={13} style={{ color: 'var(--accent-teal)' }} /> Est. Stay: ${stop.stayCost}
                </span>
              </div>
            </div>
          </div>
        ...
        <div style={{ padding: '1.25rem 1.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.85rem' }}>
            <h4 style={{ fontSize: '0.85rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
              <Navigation size={14} style={{ color: 'var(--primary)' }} /> Scheduled Activities ({stop.activities.length})
            </h4>
            <button
      ...
      {index < totalStops - 1 && (
        <div style={{
          ...
          <div style={{ width: '2px', height: '18px', background: 'var(--primary)', opacity: 0.5 }} />
          <span style={{ background: 'var(--bg-subtle)', border: '1px solid var(--border-color)', padding: '0.15rem 0.6rem', borderRadius: '12px' }}>
            Journey Route ↓
          </span>
          <div style={{ width: '2px', height: '18px', background: 'var(--primary)', opacity: 0.5 }} />
        </div>
